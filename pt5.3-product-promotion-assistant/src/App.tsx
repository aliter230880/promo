...
const defaultLLMSettings: LLMSettings = {
  ...
  temperature: 0.7,
  systemPrompt:
    "Ты senior growth marketer для indie game. Пиши коротко, конкретно и по делу. Возвращай только валидный JSON."
};
...
function makePosts(campaign: Campaign, assets: Asset[]): GeneratedPost[] {
  const assetHint = assets.length > 0 ? `Визуал: ${assets[0].name}.` : "Визуал: добавь свежий скриншот с core-механикой.";

  return campaign.channels.map((channel, index) => {
    const base = {
      steam: {
        headline: `${campaign.productName || "Игра"}: новый апдейт для wishlist-аудитории`,
        body: `Показали новый фрагмент геймплея в жанре ${campaign.genre}. Фокус: ${campaign.objective}. ${assetHint}`,
        cta: "Добавить в wishlist"
      },
      reddit: {
        headline: `Нужен фидбек по ${campaign.productName || "проекту"} от игроков ${campaign.genre}`,
        body: `Собираем реакцию от ядра аудитории: ${campaign.audience}. Что работает лучше: скорость боя или глубина прогрессии? ${assetHint}`,
        cta: "Написать мнение в комментариях"
      },
      x: {
        headline: `${campaign.productName || "Project"} devlog: короткий хук + скрин`,
        body: `Один сильный момент из билда, стиль: ${campaign.tone}. Пост под цель: ${campaign.objective}. ${assetHint}`,
        cta: "Открыть страницу и подписаться"
      },
      tiktok: {
        headline: `${campaign.productName || "Игра"}: 15-секундный gameplay hook`,
        body: `Первые 2 секунды - экшен, затем фича и payoff. Текст на экране под аудиторию: ${campaign.audience}.`,
        cta: "Ссылка в профиле"
      },
      discord: {
        headline: `${campaign.productName || "Проект"}: weekly update в комьюнити`,
        body: `Короткий changelog + вопрос к сообществу. Формируем ощущение совместной разработки и прогресса. ${assetHint}`,
        cta: "Войти на сервер"
      }
    }[channel];
...
export default function App() {
  ...
  }, [campaign, selectedRepoAssetIds, autopilotMode, llmSettings]);
